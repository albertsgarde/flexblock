...
pub fn derive_shader_id(_item : TokenStream) -> TokenStream {
    ...
    //res.push_str("#[automatically_derived]\n");
    //res.push_str("impl ShaderIdentifier {{\n");
    //res.push_str("\tpub fn extensionless_path(&self) -> &'static str  {")
   //format!("impl ShaderIdentifier {{\n\tpub fn investigate() {{ println!(\"{}\") }} }}",_item.to_string()).parse().unwrap()
}
