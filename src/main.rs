...
fn main() {
    ...
    let packing_thread =
        graphics::start_packing_thread(logic_to_packing_receiver, packing_to_window_sender, window_to_packing_receiver);

    // We unfortunately cannot catch panics from the window thread :(
    graphics::start_window(packing_to_window_receiver, window_to_logic_sender, window_to_packing_sender);
