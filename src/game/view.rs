...
impl View {
    ...
    pub fn turn(&mut self, delta: (f32, f32)) {
        ...
        let normal: Vec3 = (-delta.1*glm::cross(&self.view_direction, &self.up)-delta.0*self.up)/length;
        // See https://en.wikipedia.org/wiki/Rodrigues%27_rotation_formula#Matrix_notation.
        let k_matrix = nalgebra_glm::make_mat3x3(&[0., normal.z, -normal.y, -normal.z, 0., normal.x, normal.y, -normal.x, 0.]);
        let radians = length;
        let rotation_matrix: glm::Mat3x3 = nalgebra_glm::identity::<f32, nalgebra_glm::U3>()+radians.sin()*k_matrix + (1.-radians.cos())*k_matrix*k_matrix;
    ...
    pub fn view_to_world(&self, vec: Vec3) -> Vec3 {
        -vec.z*self.view_direction+vec.y*self.up+vec.x*glm::cross(&self.view_direction, &self.up)
    }
}
